...
export async function generateMetadata({params}:PageProps<"/[locale]/tomorrow">){const {locale}=await params;if(!isLocale(locale))notFound();return localizedMetadata(locale,"tomorrow",getMessages(locale).common.tomorrow,systemLabels[locale].tomorrowDescription)}
...
export default async function TomorrowPage({ params }: PageProps<"/[locale]/tomorrow">) {
  ...
  return <TodayExperience key={`${locale}:${date}`} data={data} locale={locale} scope="tomorrow" />;
}
