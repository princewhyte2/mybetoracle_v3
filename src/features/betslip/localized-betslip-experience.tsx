...
function Crest({ leg, side }: { leg: AccumulatorLeg; side: "home" | "away" }) {
  ...
  return <span className={styles.crest} style={{ "--crest-primary": colors[0], "--crest-secondary": colors[1] } as React.CSSProperties}>{shortName.slice(0, 2)}</span>;
}
...
export function BetslipExperience({ data, locale }: { data: DailyBetslip; locale: Locale }) {
  ...
  const savePublication=async()=>{const method=saved?"DELETE":"PUT";const response=await fetch(`/api/saved/oracle-daily/${data.id}`,{method});if(response.status===401){router.push(`/${locale}/login?returnTo=${encodeURIComponent(window.location.pathname+window.location.search)}`);return}if(response.ok)setSaved(value=>!value)};
