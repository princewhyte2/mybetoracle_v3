...
import { predictionMarkets, type Match, type MatchState, type OracleMarket, type PredictionMarket, type TodayData, type TipOutcome } from "./types";

type ApiPrediction = { predictionId: string | null; marketGroup: string; availability: string; marketType: string | null; marketValue: string | null; selection: string | null; shortSelection: string | null; confidence: number | null; odds: number | null; result?: unknown };
type ApiFixture = { id: string; matchHref: string | null; kickoffAt: string; status: { code: string; detail: string | null; elapsedMinute?: number | null }; score: { home: number | null; away: number | null }; homeTeam: { id: string; name: string; emblemUrl: string | null }; awayTeam: { id: string; name: string; emblemUrl: string | null }; predictions: ApiPrediction[] };
...
export async function getTodayData({ date, locale, page = 1, cursor, view = "all", search }: { date: string; locale: Locale; page?: number; cursor?: string; view?: "all" | "live"; search?: string }): Promise<TodayData> {
  ...
  const url = new URL(`${baseUrl}/api/v3/today`); url.searchParams.set("date", date); url.searchParams.set("timezone", "Africa/Lagos"); url.searchParams.set("locale", locale); url.searchParams.set("page", String(page)); url.searchParams.set("pageSize", "120"); url.searchParams.set("view", view); if (cursor) url.searchParams.set("cursor", cursor); if (search) url.searchParams.set("search", search);
